...
def check_interactions(payload):
        ...
        for j in range(i + 1, len(salts_present)):
            ...
            if severity:
                ...
                drug_drug_interactions.append({
                    "drugs": [a, b],
                    "brands_involved": {a: salt_sources[a], b: salt_sources[b]},
                    "severity": severity,
    ...
    if drug_drug_interactions or duplicates or drug_food_interactions:
        ...
        summary = "Some medicines could not be checked because they weren't found in our database. Nothing was flagged among the ones we could check, based on the checks in our database."
    else:
        summary = "Nothing was flagged for duplicate medicines or known interactions among the medicines checked, based on the checks in our database."
