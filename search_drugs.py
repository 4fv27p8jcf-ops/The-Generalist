...
def search_drugs(query, limit=10):
    ...
    # 4. fuzzy fallback (typo tolerance) if still not enough results
    if len(rows) < 3:  # only bother if the exact-ish tiers found almost nothing
        cur.execute("SELECT id, name, name_lower, salts, route FROM brands WHERE discontinued=0")
        all_rows = cur.fetchall()
        scored = []
        for bid, name, name_lower, salts_json, route in all_rows:
            if bid in seen_ids:
                continue
            # cheap prefilter: only consider names starting with the same letter,
            # or within a couple characters of query length - keeps this fast
            if not name_lower or name_lower[0] != q[0]:
                continue
            ratio = difflib.SequenceMatcher(None, q, name_lower[:len(q) + 3]).ratio()
            if ratio >= 0.6:
                scored.append((ratio, bid, name, salts_json, route))
        scored.sort(key=lambda x: -x[0])
        for ratio, bid, name, salts_json, route in scored:
            if bid in seen_ids:
                continue
            rows.append((bid, name, salts_json, route))
            seen_ids.add(bid)
            if len(rows) >= limit:
                break
